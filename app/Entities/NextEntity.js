...
class NextEntity {
  ...
  async getTracking(id) {
    let tracking = await Tracking.query()
      ...
      .where(
        "trackings.dependencia_id",
        this.is_externo ? null : this.dependencia_id
      )
    ...
    if (this.hidden.includes(tracking.status)) {
      if (tracking.visible == 1)
        throw new CustomException(`El trámite ya fué verificado`);
    } else {
      if (tracking.visible == 0)
        throw new CustomException(`El trámite aún no está verificado`);
    }
    // agregar tracking
  ...
  async getTrackingOrigen() {
    ...
    if (!origen)
      throw new CustomException("No se encontró la dependencia de origen");
    this.tracking_origen = origen;
  }

  async getUser(id) {
    let response = await this.authentication
      .get(`user/${id}`)
      .then((res) => res.data)
  ...
  async generateSelfRoles() {
    ...
    this.current_role = await this.getRole(
      this.dependencia_id,
      null,
      this.user_id
    );
  ...
  async generateInfo({ onlyDescription = true }, callback = null) {
    ...
    if (
      (!onlyDescription && (files || description)) ||
      ...
      if (files) {
        ...
        await apiFile.store(
          this.request,
          infoFile,
          `tramite/${this.tramite.slug}/info`,
          this.trx
        );
      }
  ...
  async validateStatus(status) {
    if (!this.tracking.next) {
      ...
      if (typeof current_action == "undefined")
        throw new Error("No se puede procesar el trámite");
      if (!current_action.includes(status || ""))
        throw new ValidatorError([
          { field: "status", message: "El status es invalido!" },
        ]);
      this.status = status;
    } else {
      if (this.tracking.next != status)
        throw new Error(`La acción "${status}" es incorrecta!`);
      this.status = status;
    }
  }
  ...
  async disableTrackingCurrent() {
    ...
    if (!this.tracking.readed_at)
      payload.readed_at = moment().format("YYYY-MM-DD hh:mm:ss");
    // preparate
    this.tracking.merge(payload);
    await this.tracking.save();
  }

  async disabledVisible(
    dependenciaIds = [],
    trackingIds = [],
    userVerifyIds = []
  ) {
  ...
  async deleteFilesNotPdf() {
    ...
    for (let file of files) {
      ...
      if (existsFile) {
        await Drive.delete(file.real_path);
      }
      // eliminar regístro
  ...
  async count_file_observer() {
    let count = await File.query()
      ...
      .getCount("id");
    if (count)
      throw new CustomException("Existe uno o varios archivos observados");
    return true;
  }
  ...
  async canAllowAction() {
    if (this.is_privated && this.user_id != this.tracking.user_verify_id) {
      ...
      if (!Object.keys(this.current_role || {}).length)
        new CustomException(
          "Usted no cuenta con un rol para derivar el trámite fuera de la dependencia"
        );
      if (
        this.current_role.status != "BOSS" &&
        this.tracking.modo != "DEPENDENCIA"
      )
        throw new CustomException(
          "Usted no puede derivar el trámite fuera de la dependencia"
        );
    }
    return true;
  }
  ...
  enviado = async () => {
    ...
    this.is_privated =
      this.body.dependencia_destino_id == this.tracking.dependencia_id;
    if (this.is_privated) {
      await validation(null, this.body, { user_destino_id: "required" });
      // validar si el usuario destino es el mismo
      if (this.user_id == this.body.user_destino_id)
        throw new ValidatorError([
          {
            field: "user_destino_id",
            message: "Usted no puede ser el usuario destino",
          },
        ]);
    }
    ...
    this.destino_boss = await this.getRole(
      payload_recibido.dependencia_id,
      "BOSS"
    );
    ...
    if (!this.is_privated) {
      ...
      payload_recibido.user_verify_id = this.current_user.id;
      payload_recibido.person_id = this.current_user.person_id;
      payload_recibido.modo = await this.isModoBoss(
        this.destino_boss,
        payload_recibido
      );
    }
    ...
    payload_derivado.modo = await this.isModoBoss(
      this.current_boss,
      payload_derivado
    );
    // procesar trámite
    try {
      ...
      // multiples
      if (!this.is_privated) {
        await this.multipleDependencias(recibido, derivado);
      } else {
        await this.multipleUsers(recibido, derivado);
      }
  ...
  respondido = async () => {
    ...
    try {
      ...
      payload_respondido.modo = await this.isModoBoss(
        this.current_boss,
        payload_respondido
      );
      // crear recibido
  ...
  aceptado = async () => {
    ...
    let payload_aceptado = {
      ...
      first: 0,
      status: "ACEPTADO",
      readed_at: this.tracking_origen.first
        ? moment().format("YYYY-MM-DD hh:mm:ss")
        : null,
    };
    // obtener jefe del area del documento
    this.destino_boss = await this.getRole(
      payload_aceptado.dependencia_id,
      "BOSS"
    );
    payload_aceptado.modo = this.tracking_origen.dependencia_id
      ? await this.isModoBoss(this.destino_boss, payload_aceptado)
      : "DEPENDENCIA";
    ...
    try {
      ...
      payload_pendiente.modo = await this.isModoBoss(
        this.current_boss,
        payload_pendiente
      );
      // crear pendiente
  ...
  rechazado = async () => {
    ...
    // obtener jefe del area del documento
    this.destino_boss = await this.getRole(
      payload_pendiente.dependencia_id,
      "BOSS"
    );
    payload_pendiente.modo = this.tracking_origen.dependencia_id
      ? await this.isModoBoss(this.destino_boss, payload_pendiente)
      : "DEPENDENCIA";
    ...
    try {
      ...
      payload_rechazado.modo = await this.isModoBoss(
        this.current_boss,
        payload_rechazado
      );
      // crear rechazado
  ...
  multipleDependencias = async (next_tracking, current_tracking) => {
    ...
    let roles = Role.query().where("entity_id", this.entity.id);
    // filtrar dependencia
    if (next_tracking.dependencia_id)
      roles.whereNotIn("dependencia_id", [next_tracking.dependencia_id]);
    // obtener datos
    roles = await roles
