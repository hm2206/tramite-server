...
class TramiteEntity {
  ...
  async index(tmpDatos = this.dataPagination) {
    ...
    let tramites = Tramite.query()
      .with("current_tracking", (build) => {
        build.where("current", 1).where("visible", 1);
      })
      .withCount("current_tracking")
      .with("tramite_type");
    // filtrar query_saerch
    if (datos.query_search)
      tramites.where(
        DB.raw(
          `(slug like '%${datos.query_search}%' OR asunto like '%${datos.query_search}%')`
        )
      );
    // filtros
    for (let attr in datos.custom) {
      ...
      if (Array.isArray(value)) tramites.whereIn(attr, value);
        ...
        tramites.where(DB.raw(attr), value);
    }
    let isPaginate = datos.perPage ? true : false;
    // obtener datos
    tramites = isPaginate
      ? await tramites.paginate(datos.page, datos.perPage)
      : await tramites.fetch();
    tramites = await tramites.toJSON();
    let result = collect(isPaginate ? tramites.data : tramites);
    let dependenciaIds = result
      .groupBy("dependencia_origen_id")
      .keys()
      .toArray();
    let peopleIds = result.groupBy("person_id").keys().toArray();
    // obtener dependencias
    let dependencias = await this.authentication
      .get(`dependencia?ids[]=${dependenciaIds.join("&ids[]=")}`)
      .then(({ data }) => data.dependencia.data || [])
      .catch((err) => []);
    dependencias = collect(dependencias);
    // obtener people
    let people = await this.authentication
      .get(`person?ids[]=${peopleIds.join("&ids[]=")}`)
    ...
    await result.map((tra) => {
      ...
      tra.dependencia =
        dependencias.where("id", tra.dependencia_origen_id).first() || {};
      return tra;
    });
    // add tramites
  ...
  async show(id, column = "id") {
    ...
    if (!allowColumn.includes(column))
      throw new CustomException("La columna no está permitida!");
    ...
    tramite.files = await tramite
      .files()
      .where("object_type", "App/Models/Tramite")
      .fetch();
    tramite = JSON.parse(JSON.stringify(tramite));
    // obener persona
    let { person } = await this.authentication
      .get(`person/${tramite.person_id}`)
      .then((res) => res.data)
      .catch(() => ({ person: {} }));
    // obtener dependencia
    let { dependencia } = await this.authentication
      .get(`dependencia/${tramite.dependencia_origen_id}`)
      .then((res) => res.data)
      .catch(() => ({ dependencia: {} }));
    // add datos
  ...
  async store(request, datos = this.attributes, auth = {}, next = null) {
    ...
    // obtener persona
    let { person, success } = await this.authentication
      .get(`person/${datos.person_id || "_error"}`)
      .then((res) => res.data)
      .catch(() => ({ success: false }));

    if (!success) {
      throw new ValidatorError([
        { field: "person_id", message: `La persona no existe!` },
      ]);
    }
    ...
    try {
      ...
      if (next && !allow.includes(next))
        throw new CustomException(
          `la siguiente acción "${next}" no está permitida!`
        );
      // obtener boss
      if (!self_remitente) {
      ...
      if (tramite.tramite_parent_id) {
        ...
        if (!before_tracking)
          throw new CustomException(
            `No se encontró un seguimiento activo del trámite raíz!`
          );
      ...
      let files = await fileEntity.store(
        request,
        {
      ...
      if (!tramite.dependencia_origen_id) {
        ...
        const nextEntity = new NextEntity(
          authentication,
          datos.entity_id,
        ...
        await nextEntity.execute(tracking.id, "ENVIADO", {
          dependencia_destino_id: datos.dependencia_id,
        });
      }
      // response
  ...
  async update(id, datos = this.attributes, user_id = 0) {
    ...
    if (user_id) {
      if (tramite.user_id != user_id && tramite.user_verify_id != user_id)
        throw new CustomException(
          "Usted no está permitido a guardar los cambios del trámite"
        );
    }
    // preparar datos
    tramite.merge({
