...
class TramitePublicController {
  ...
  store = async ({ request }) => {
    ...
    // obtener entity
    let { entity } = await authentication
      .get(`entity/${request.input("entity_id")}`)
      .then((res) => res.data)
      .catch(() => ({ success: false, entity: {} }));
    // validar entity
    if (!entity.id)
      throw new ValidatorError([
        { field: "entity_id", message: "No se encontró la entidad" },
      ]);
    // obtener dependencia
    let { dependencia } = await authentication
      .get(`dependencia/${request.input("dependencia_id")}`)
      .then((res) => res.data)
      .catch(() => ({ success: false, dependencia: {} }));
    // validar dependencia
    if (!dependencia.id)
      throw new ValidatorError([
        { field: "dependencia_id", message: "No se encontró la dependencia" },
      ]);
      ...
      throw new ValidatorError([
        {
          ...
          message: "La dependencia no está permitida",
        },
      ]);
    // processar trámite
    datos.slug = uid(10);
  ...
  show = async ({ params, request }) => {
    ...
    // obtener entity
    let entity = await request.api_authentication
      .get(`entity/${tramite.entity_id}`)
      .then((res) => res.data)
      .catch((err) => ({}));
    // add entity
    tramite.entity = entity;
    // obtener dependencia
    let { dependencia } = await request.api_authentication
      .get(`dependencia/${tramite.dependencia_id}`)
      .then((res) => res.data)
      .catch((err) => ({
        success: false,
        message: err.message,
    ...
    let origen = await request.api_authentication
    ...
    // obtener persona
    let person = await request.api_authentication
      .get(`find_person/${tramite.person_id}`)
      .then((res) => res.data)
      .catch((err) => ({}));
    // add persona
  ...
  tracking = async ({ params, request }) => {
    ...
    let idDestinos = await collect(tracking.data)
      .pluck("dependencia_destino_id")
      .all()
      .join("&ids[]=");
    let destino = await request.api_authentication
      .get(`dependencia?ids[]=${idDestinos}`)
      .then((res) => res.data)
      .catch((err) => ({
        success: false,
    ...
    let user = await request.api_authentication
      .get(`user?ids[]=${userIds}`)
      .then((res) => res.data)
      .catch((err) => ({ data: [] }));
    // collect user
    ...
    tracking.data.map(async (tra) => {
      tra.dependencia_destino =
        destino.where("id", tra.dependencia_destino_id).first() || {};
