...
class TramiteTypeController {
  index = async ({ request }) => {
    ...
    // get tramite_type
    let tramite_type = TramiteType.query().where("state", state);
    // filtros
    if (query_search)
      tramite_type.whereRaw(
        `(short_name LIKE '%${query_search}%' OR nombre LIKE '%${query_search}%')`
      );
    // paginación
  ...
  update = async ({ params, request }) => {
    ...
    // validar short_name
    if (
      await TramiteType.query()
        .whereRaw(
          `id <> ${params.id} AND short_name = '${request.input("short_name")}'`
        )
        .getCount()
    )
