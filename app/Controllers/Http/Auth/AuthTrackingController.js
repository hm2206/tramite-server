...
class AuthTrackingController {
  ...
  _dependencias = async (request, trackings) => {
    ...
    let pluckedTrackingSend = db
      .pluck("tracking_send.dependencia_id")
    ...
    let { dependencia } = await request.api_authentication
      .get(`dependencia?ids[]=${ids.join("&ids[]=")}`)
      .then((res) => res.data)
      .catch((err) => ({
        success: false,
  ...
  _people = async (request, trackings) => {
    ...
    let people = await request.api_authentication
      .get(`find_people?id[]=${ids.join("&id[]=")}`)
      .then((res) => res.data)
      .catch((err) => []);
    // response
  ...
  handle = async ({ params, request }) => {
    ...
    let modos =
      `${params.modo}`.toUpperCase() == "DEPENDENCIA"
        ? ["DEPENDENCIA"]
        : ["DEPENDENCIA", "YO"];
    status =
    ...
    if (typeof archived != "undefined")
      trackings.where("trackings.archived", archived);
    if (tracking_id.length) trackings.whereIn("trackings.id", tracking_id);
    if (modos.includes("YO"))
      trackings.where("trackings.user_verify_id", auth.id);
    if (status.length) trackings.whereIn("trackings.status", status);
    if (query_search)
      trackings
        .leftJoin("files as f", "f.object_id", "tra.id")
        .where("f.object_type", "App/Models/Tramite")
        .where(
          DB.raw(
            `(tra.slug like '%${query_search}%' OR tra.document_number like '%${query_search}%' OR f.name like '%${query_search}%')`
          )
        )
        .groupBy(
          "trackings.id",
          "trackings.tramite_id",
          "trackings.dependencia_id",
          "trackings.dependencia_id",
          "trackings.user_id",
          "trackings.user_verify_id",
          "trackings.person_id",
          "trackings.current",
          "trackings.alert",
          "trackings.revisado",
    ...
    await trackings.data.map(async (d, indexD) => {
      let current_dependencia =
        (await dependencias.where("id", d.dependencia_id || "").first()) || {};
      // config tracking
      ...
      if (validation) {
        ...
        if (current_tracking) {
          d.dependencia =
            (await dependencias
              .where("id", current_tracking.dependencia_id || "")
              .first()) || {};
        }
      }
      // config trámite
      d.tramite.dependencia_origen =
        (await dependencias
          .where("id", d.tramite.dependencia_origen_id || "")
          .first()) || {};
      d.tramite.person =
        (await people.where("id", d.tramite.person_id).first()) || {};
      d.tramite.files =
        (await files
          .where("object_type", "App/Models/Tramite")
          .where("object_id", d.tramite.id)
          .toArray()) || [];
      d.is_next = true;
      // validar seleccion
      if (configs_keys.includes(d.status)) {
        let current_config = configs.where("key", d.status).first() || {};
        configs_index[d.status] =
          typeof configs_index[d.status] === "number"
            ? configs_index[d.status] + 1
            : 1;
