...
class AuthTramiteController {
  ...
  index = async ({ request }) => {
    ...
    if (query_search)
      tramites
        .leftJoin("files as f", "f.object_id", "tramites.id")
    ...
    if (tramite_type_id)
      tramites.join("tramite_types as tra").where("tra.id", tramite_type_id);
    // onbtener datos
    tramites = await tramites.select("tramites.*").paginate(page || 1, 20);
  ...
  show = async ({ params, request }) => {
    ...
    let dependenciaIds = [
      tracking.dependencia_id,
      tramite.dependencia_origen_id,
      tracking.tracking ? tracking.tracking.dependencia_id : "",
    ];
    ...
    let personIds = [
      tracking.person_id,
      tramite.person_id,
      tracking.tracking ? tracking.tracking.person_id : null,
    ];
    let people = await this._people(request, personIds);
    // setting tracking
    tracking.person = people.where("id", tracking.person_id).first() || {};
    tracking.dependencia =
      dependencias.where("id", tracking.dependencia_id || "").first() || {};
    // original
    ...
    if (tracking.tracking) {
      tracking.person =
        people.where("id", tracking.tracking.person_id).first() || {};
      tracking.dependencia =
        dependencias
          .where("id", tracking.tracking.dependencia_id || "")
          .first() || {};
      tracking.tracking.person = tracking.person;
      tracking.tracking.dependencia = tracking.dependencia;
    }
    // setting tramite
    tracking.tramite.person =
      people.where("id", tracking.tramite.person_id).first() || {};
    tracking.tramite.dependencia_origen =
      dependencias
        .where("id", tracking.tramite.dependencia_origen_id)
        .first() || {};
    // archivos
    let files = await this._files([tracking.id, tracking.tramite_id]);
    // add files
    tracking.tramite.files = await files
      .where("object_type", "App/Models/Tramite")
      .where("object_id", tracking.tramite_id)
      .toArray();
  ...
  _dependencias = async (request, ids = []) => {
    let dependencias = await request.api_authentication
  ...
  _files = async (
    ids = [],
    types = ["App/Models/Tracking", "App/Models/Tramite"]
  ) => {
